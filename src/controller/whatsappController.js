...
export default class WhatsAppController {
      ...
      this._firebase.initAuth()
         .then(response => {
            ...
            this._user.on('datachange', data => {
               ...
               document.querySelector('title').innerHTML = `WhatsApp - ${data.name}`;
   ...
   elementsPrototype() {
      ...
      Element.prototype.on = function(events, fnc) {
         events.split(' ').forEach(event => {
            this.addEventListener(event, fnc);
         });
